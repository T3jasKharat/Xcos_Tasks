...

      {
        Object.keys(val).map((keyName, i) => {
          if (keyName.match(/^p[0-9]*_value$/)) {
            ...
            if (compProperties !== undefined && compProperties[rootKeyName] !== null && compProperties[typeId] !== null && compProperties[helpId] !== null) {
              return (
                <ListItem key={i}>
                  <TextField id={keyName} label={compProperties[rootKeyName]} value={val[keyName] || ''} size='small' variant='outlined' onChange={getInputValues} helperText={compProperties[helpId]} />
                </ListItem>
              )
            }
          }
